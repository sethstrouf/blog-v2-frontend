<script lang="ts">
    ...
    const next = () => {
      index = (index + 1) % data.post.attributes.images.length
    }
    ...
    const previous = () => {
      if (index > 0) {
        ...
        index = data.post.attributes.images.length - 1
      }
    }
</script>
      ...
      <div class="mx-auto grid max-w-2xl grid-cols-1 items-start gap-x-8 gap-y-16 sm:gap-y-24 lg:mx-0 lg:max-w-none lg:grid-cols-2">
        <div class="lg:pr-4 mx-auto">
          <figure class="relative block lg:hidden">
            {#each [data.post.attributes.images[index]] as image (index)}
          ...
          {#each data.post.attributes.images as image}
        ...
        <div>
          <div class="text-base leading-7 text-gray-700 lg:max-w-lg -mt-12 sm:-mt-20 lg:mt-0">
            <p class="text-base font-semibold leading-7 text-indigo-600">{getDate(new Date(data.post.attributes.created_at))}</p>
            <h1 class="mt-2 font-bold tracking-tight">{data.post.attributes.title}</h1>
            <div class="max-w-xl">
          ...
          <div class="mt-10 flex border-t">
            <p class="mt-8 text-base font-semibold leading-7 text-indigo-600">Comment <span aria-hidden="true">&rarr;</span></p>
          </div>
        </div>
      </div>
    </div>
